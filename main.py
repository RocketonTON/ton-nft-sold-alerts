...
async def royalty_trs(royalty_address):
    ...
    try:
        ...
        # 2. OTTIENI TRANSAZIONI (ORA ASINCRONO)
        print(f"[royalty_trs] Fetching transactions for {royalty_address[-6:]}...", flush=True)
        transactions = await get_transactions_http(royalty_address, limit=50)
        ...
        for tx in reversed(transactions):
            ...
            for method in get_methods:
                # ORA ASINCRONO
                stack = await run_get_method_http(source, method)
                ...
                # ORA ASINCRONO
                nft_data = await get_nft_data_http(nft_address)
                
                if not nft_data or nft_data[1] not in collections_list:
                    continue
                
                # get_collection_floor è SINCRONA, ma wrappata
                collection_floor_data = await asyncio.to_thread(get_collection_floor, nft_data[1])
