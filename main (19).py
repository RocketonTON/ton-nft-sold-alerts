...
async def get_global_client():
    ...
    if _ton_client is None:
        ...
        # LiteBalancer con timeout
        _ton_client = LiteBalancer.from_mainnet_config(trust_level=2, timeout=30)
        
        try:
            # Timeout di 60 secondi per la connessione iniziale
            await asyncio.wait_for(_ton_client.start_up(), timeout=60)
            print("[INIT] ✅ Global LiteBalancer client created successfully.")
        except asyncio.TimeoutError:
            print("[INIT] ⚠️ Timeout durante start_up, riprovo...")
            _ton_client = None
            # Riprova con configurazione più semplice
            _ton_client = LiteBalancer.from_mainnet_config(trust_level=0, timeout=15)
            await asyncio.wait_for(_ton_client.start_up(), timeout=45)
            print("[INIT] ✅ Client connesso con trust_level=0")
        except Exception as e:
            print(f"[INIT] ❌ Errore critico nella creazione del client: {e}")
            _ton_client = None
            raise
    return _ton_client
...
async def royalty_trs(royalty_address):
    ...
    try:
        ...
        # 4. PRENDI LE TRANSAZIONI CON RETRY
        trs = None
        max_retries = 3
        for attempt in range(max_retries):
            try:
                # Timeout di 30 secondi per get_transactions
                trs = await asyncio.wait_for(
                    client.get_transactions(address=addr, count=trs_limit),
                    timeout=30
                )
                print(f"[royalty_trs] Trovate {len(trs) if trs else 0} transazioni per {royalty_address[-6:]}")
                break  # Successo, esci dal loop
            except asyncio.TimeoutError:
                print(f'[royalty_trs] Timeout tentativo {attempt + 1}/{max_retries} per {royalty_address[-6:]}')
                if attempt < max_retries - 1:
                    await asyncio.sleep(2)  # Aspetta 2 secondi prima di riprovare
                else:
                    print(f'[royalty_trs] Tutti i tentativi falliti per {royalty_address[-6:]}')
                    return None
            except Exception as e:
        ...
        if trs:
            for tr in reversed(trs):  # Dalla più vecchia alla più nuova
                ...
                for method in get_methods:
                    try:
                        # Timeout anche per run_get_method
